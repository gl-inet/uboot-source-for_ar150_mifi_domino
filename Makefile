...
domino:	export UBOOT_FILE_NAME=uboot-gl-ar150
domino:	export MAX_UBOOT_SIZE=256
#domino:	export DEVICE_VENDOR=domino
domino:
	@cd $(BUILD_TOPDIR)/u-boot/ && $(MAKECMD) domino_config
	@cd $(BUILD_TOPDIR)/u-boot/ && $(MAKECMD) ENDIANNESS=-EB V=1 all
	@cp $(BUILD_TOPDIR)/u-boot/u-boot.bin $(BUILD_TOPDIR)/bin/temp.bin
	@make show_size
